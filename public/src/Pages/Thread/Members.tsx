...
export default function Members({ post, user, sendAdvancedRequest }: { post: Thread | string, user: User | null, sendAdvancedRequest: (url: string, options?: Object) => void }) {

    function onClickButton(type: "makeModerator" | "removeModerator" | "removeMember", user: User) {
        switch (type) {
            case "makeModerator":
            ...
            case "removeModerator":
                ...
                break;
            case "removeMember":
                alert("Remove a member", `Are you sure you want to remove ${user.username} as a member? This action cannot be undone.`, () => sendAdvancedRequest(`/api/thread/{id}/kick`, { method: "POST", body: JSON.stringify({ userId: user.id }), headers: { "Content-Type": "application/json" } }));
                break;
        }
    }
    ...
    return (<div className="members">
        <div className="members-header">
            <div className="members-header-title">Members</div>
        </div>
        <div className="members-body">
            <div className="members-body-list">
                {typeof post !== "string" && post.members.map((member) => {
                    return (<div className="members-body-list-item">
                        ...
                        <div className="members-body-list-item-buttons">
                            ...
                            {post.moderators.find(u => u.id === user?.id) && post.members.find(u => u.id === member.id) && <button className={`members-body-list-item-buttons-red-member ${post.author.id === member.id || member.id === user?.id ? "members-body-list-item-buttons-disabled" : ""}`} onClick={() => onClickButton("removeMember", member)}>Remove Member</button>}
                        </div>
                    </div>)
                })}
            </div>
