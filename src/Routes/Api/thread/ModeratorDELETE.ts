...
export default class ModeratorDELETE extends BaseAPI {
    constructor(db: DatabaseManager) {
        super("/:id/moderator", db, "delete");
    }
    ...
    async run(this: Api, req: Request, res: Response, next: NextFunction): Promise<any> {
        ...
        if (req.user.id === userId) return res.status(400).json({ error: true, message: "You can't remove yourself as a moderator" });
        ...
        if (!thread.moderators.find(u => u.id === userId)) return res.status(400).json({ error: true, message: "User is not a moderator" });

        const _thread = await this.database.removeModeratorFromThreadById(id, user).catch(_ => null);
        if (!_thread) return res.status(500).json({ error: true, message: "An error occured while removing the moderator" });

        return res.json({ error: false, message: "Successfully removed the moderator", thread: _thread });
    }
}